...
var GeometrizeWorkerInterface = function() {
	var _gthis = this;
	this.worker = new Worker("static/js/geometrize_worker.js");
...
Main.prototype = {
	...
	onWindowLoaded: function() {
		...
		setShapeOption(0, true);
		setShapeOption(1, false);
		setShapeOption(2, false);
		setShapeOption(3, false);
		setShapeOption(4, false);
		setShapeOption(5, true);
		setShapeOption(6, false);
		setShapeOption(7, false);

		document.getElementById('rectangles').addEventListener('click', (e) => {
			setShapeOption(0, e.target.checked);
		});
		document.getElementById('circles').addEventListener('click', (e) => {
			setShapeOption(5, e.target.checked);
		});
	...
	stepRunner: function() {
		var options = {
			shapeTypes: this.shapeTypes.length == 0 ? [0, 5] : geometrize_ArraySet.toArray(this.shapeTypes),
	...
	onTargetImageChanged: function() {
		...
		this.shapeSvgData = [];
		this.shapeJsonData = [];
		this.appendSvgShapeData(geometrize_exporter_SvgExporter.exportShape({
			score: 0.0,
			color: premultipliedColor,
			shape: backgroundRect
		}));
		this.shapeJsonData.push(geometrize_exporter_ShapeJsonExporter.exportShape({
			score: 0.0,
			color: premultipliedColor,
			shape: backgroundRect
		}));
	...
	set_running: function(running) {
		...
		function fixCheckbox(id) {
			let cb = document.getElementById(id);
			cb.click();
			cb.click();
		}
		fixCheckbox('rectangles');
		fixCheckbox('circles');
...
geometrize_exporter_SvgExporter.getSvgNodeOpen = function(width, height) {
	return "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.2\" baseProfile=\"tiny\" height=\"100%\" viewBox=\"0 0 " + width + " " + height + "\">\n";
};
...
geometrize_shape_RotatedRectangle.prototype = {
	rasterize: function() {
		...
		return geometrize_rasterizer_Scanline.trim(geometrize_rasterizer_Rasterizer.scanlinesForPolygon([{
			x: ulx,
			y: uly
		}, {
			x: urx,
...
var geometrize_shape_ShapeTypes = function() {};
...
haxe_ds_IntMap.prototype = {
	keys: function() {
		...
		for (var key in this.h)
			if (this.h.hasOwnProperty(key)) a.push(+key);
...
function $bind(o, m) {
	...
	if (m.__id__ == null) m.__id__ = $haxeUID++;
	var f;
